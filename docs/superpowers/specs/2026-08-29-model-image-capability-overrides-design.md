...
首版运行时只使用不会产生额外外部调用的可信信息：pi-ai 已安装目录中的能力、
Windows wrapper 已验证的 OpenCode 基础目录，以及 Provider profile 的
`defaultInput`。这些来源由 pi-ai 在模型解析时消费，不复制到 Models 浏览器插件。

当前 `llm.models` 浏览器 RPC 和 `DiscoveredModelView` 都不携带图片模态，因此首版
页面不能可靠显示某个自动模型的最终图片能力。扩展上游 RPC 不属于本次本地改动；
界面只解释自动优先级和未知时的安全回退，不伪装成已经读到了运行时能力。

现有 `/models` 发现结果只负责列出候选模型和容量。新候选默认保持 `自动`，不会
把模型名称或其他非标准字段猜成 `input`。接口未提供模态时不报“识别失败”，而是
明确说明自动结果由重启后的运行时目录或 Provider 默认决定。
...
- 自动：`由模型目录或 Provider 默认决定；无法确认时按仅文本处理`；
- 非自动状态：`模型条目明确声明支持图片`或`模型条目明确声明仅文本`。

提示同时说明保存并重启后重新解析，不能把浏览器拿不到的运行时能力显示成已验证
结果。
...
CPA 原生编辑 normalization 只设置 Provider 级 `defaultInput: ['text', 'image']`，
必须保留模型级缺失或空 `input` 作为自动，也必须保留非法原值交给通用保存校验拒绝；
不得在校验前把两者改写为图片。只有启动迁移确认旧 CPA Profile 尚未具备当前
Provider 图片默认值时，才为历史上缺失或空的模型模态物化 `['text', 'image']`。
迁移完成后，用户再次选择自动不会在后续启动中被重新物化；显式仅文本覆盖始终保留。
...
- 中英文标签、帮助文案和键盘可访问状态完整；自动状态不得声称已读取图片能力。
