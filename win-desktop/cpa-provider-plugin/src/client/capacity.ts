...
export function mergeCapacityDrafts(current: CpaCapacityDrafts, discovered: readonly CpaModelCandidate[]): Map<string, CpaCapacityDraft> {
  const merged = new Map(current)
  for (const model of discovered) if (!merged.has(model.id)) merged.set(model.id, draftFromModel(model))
  return merged
}

export function applyCapacityDrafts(models: readonly CpaModelCandidate[], drafts: CpaCapacityDrafts): CpaCapacityResult {
  const parsedModels: CpaModelCandidate[] = []
  for (const model of models) {
    if (model.selected === false) { parsedModels.push(model); continue }
