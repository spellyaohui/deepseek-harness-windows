...
## 桌面 fork `0.1.2-alpha.2-desktop.1`

本桌面 fork 为每条 pi-ai 模型行增加提供方无关的图像输入选择：**自动**、**文本和图像**、**仅文本**。选择只写入该模型自己的 `input` 字段；自动会删除这个字段，让已安装的提供方目录继续负责能力判断，另外两种选择记录对应的输入模态列表。按提供方提供的批量操作只修改当前草稿，并保留其他模型字段。

非法模态数据会显示为无效并阻止应用，直到用户明确选择有效状态；不会过滤或静默降级。未知模型在自动模式下继续按仅文本处理，保存后必须重启，新的模型级覆盖才会进入运行时。

同一个模型编辑器还提供提供方无关的**能力探测**。用户可勾选一条或多条模型行，按照当前表单中的 API 地址和明确选择的协议执行有界探测。已保存的凭据只会在 Host 侧通过既有凭据 seam 解析，新输入的密钥只用于本次请求；探测不会把任何凭据返回到浏览器或写入配置。探测覆盖文本／图像输入、思考强度写法、developer 角色、严格工具、store、流式 usage 和输出 token 字段兼容性。成功或明确不支持的结果只会更新未保存草稿；已有模型能力默认保留，只有明确勾选**覆盖已有能力配置**才会覆盖。临时 HTTP 失败会保留为无法确认。探测按选择顺序串行执行并可取消，保存提供方仍然是唯一的设置写入点。
