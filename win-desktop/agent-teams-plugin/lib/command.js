...
export function buildActivationDirective(goal, profile, taskPlanning = 'seed') {
    ...
    if (profile !== undefined) {
        ...
        if (taskPlanning === 'captain') {
            lines.push('This profile supplies the roster and guardrails. Keep the plan staged after create and do not recreate members.', 'Derive the smallest useful task graph from the goal in the staged plan, then let the user review the Web plan before approval; do not ask the user whether to split, merge, serialize, or parallelize.', 'Independent supplemental work must become separate ready tasks so idle members can run in parallel. Add dependencies only for genuine prerequisites and later synthesis.');
        }
        else {
