...
export function buildActivationDirective(goal, profile, taskPlanning = 'seed') {
    ...
    if (profile !== undefined) {
        ...
        if (taskPlanning === 'captain') {
            lines.push('This profile supplies the roster and guardrails. After create, build the smallest useful task graph with captain-owned AgentTeams task tools; do not recreate members or use the Web staged-plan review for ordinary delegation.', 'Do not ask the user to name, split, merge, serialize, parallelize, or approve the plan. Add dependencies only for genuine prerequisites and later synthesis.', 'Independent supplemental work must become separate ready tasks so idle members can run in parallel.');
        }
        else {
