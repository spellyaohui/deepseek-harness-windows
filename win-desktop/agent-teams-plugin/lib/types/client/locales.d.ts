/** `agentTeams` namespace dictionaries for every plugin-owned Web surface. */
/** Dictionary namespace owned by the AgentTeams client plugin. */
export declare const AGENT_TEAMS_LOCALE_NAMESPACE = "agentTeams";
/** Simplified Chinese dictionary (the key-set source of truth). */
export declare const zh: {
    'card.memberCount': string;
    'action.openActivityPanel': string;
    'activity.panelButton': string;
    'activity.badgeAria': string;
    'activity.panelAria': string;
    'activity.title': string;
    'activity.float': string;
    'activity.dockRight': string;
    'activity.collapse': string;
    'activity.empty': string;
    'team.stop': string;
    'team.stopped': string;
    'team.stopTitle': string;
    'team.stopDescription': string;
    'team.stopCancel': string;
    'team.stopConfirm': string;
    'team.stopping': string;
    'team.stopFailed': string;
    'team.stopRequestFailed': string;
    'team.discarded': string;
    'format.listSeparator': string;
    'task.status.pending': string;
    'task.status.claimed': string;
    'task.status.inProgress': string;
    'task.status.completed': string;
    'task.status.failed': string;
    'task.status.cancelled': string;
    'task.status.notRun': string;
    'member.state.working': string;
    'member.state.failed': string;
    'member.state.waiting': string;
    'member.state.delivered': string;
    'member.state.left': string;
    'member.state.removed': string;
    'member.state.pending': string;
    'member.state.unassigned': string;
    'member.state.staged': string;
    'member.state.notCreated': string;
    'member.state.stopped': string;
    'member.status.executing': string;
    'member.status.executingModel': string;
    'member.status.working': string;
    'member.status.waitingOn': string;
    'member.status.waitingPrerequisite': string;
    'member.status.waitingAssignment': string;
    'member.status.delivered': string;
    'member.status.idle': string;
    'member.status.unknown': string;
    'member.status.staged': string;
    'member.status.settled': string;
    'member.status.discarded': string;
    'member.status.stopped': string;
    'task.assignee.unclaimed': string;
    'task.summary.waitingBreakdown': string;
    'task.summary.staged': string;
    'task.summary.discarded': string;
    'task.summary.allDelivered': string;
    'task.summary.ended': string;
    'task.summary.blockedAndRunning': string;
    'task.summary.more': string;
    'task.summary.running': string;
    'task.summary.ready': string;
    'task.summary.blocked': string;
    'task.summary.failedSettled': string;
    'task.summary.waitingSchedule': string;
    'progress.aria': string;
    'progress.title': string;
    'progress.running': string;
    'progress.blocked': string;
    'progress.delivered': string;
    'dependency.aria': string;
    'dependency.parallel': string;
    'dependency.title': string;
    'dependency.hint.parallel': string;
    'dependency.hint.chain': string;
    'dependency.hint.pinned': string;
    'task.runningAria': string;
    'task.model': string;
    'member.model': string;
    'task.detail.completed': string;
    'task.detail.noPrerequisite': string;
    'task.detail.ready': string;
    'task.detail.waitingOn': string;
    'task.detail.notRun': string;
    'task.detail.noDownstream': string;
    'task.detail.unlocks': string;
    'team.ended': string;
    'plan.badge': string;
    'plan.title': string;
    'plan.description': string;
    'plan.member.role': string;
    'plan.member.provider': string;
    'plan.member.model': string;
    'plan.member.reasoning': string;
    'plan.member.reasoningHint': string;
    'plan.model.choose': string;
    'plan.model.currentUnavailable': string;
    'plan.model.route': string;
    'plan.model.defaultReasoning': string;
    'plan.model.providerDefault': string;
    'plan.model.modelDefault': string;
    'plan.model.triggerAria': string;
    'plan.model.back': string;
    'plan.model.loading': string;
    'plan.model.empty': string;
    'plan.model.partialFailure': string;
    'plan.model.retry': string;
    'plan.member.prompt': string;
    'plan.member.roleFallback': string;
    'plan.task.subject': string;
    'plan.task.description': string;
    'plan.task.assignee': string;
    'plan.task.dependencies': string;
    'plan.task.dependenciesHint': string;
    'plan.task.kind': string;
    'plan.task.kind.work': string;
    'plan.task.kind.requirements': string;
    'plan.task.kind.implementation': string;
    'plan.task.kind.verification': string;
    'plan.task.kind.review': string;
    'plan.task.kind.repair': string;
    'plan.task.kind.integration': string;
    'plan.task.round': string;
    'plan.task.objective': string;
    'plan.task.inScope': string;
    'plan.task.outOfScope': string;
    'plan.task.acceptance': string;
    'plan.task.verify': string;
    'plan.task.deliverables': string;
    'plan.task.nonGoals': string;
    'plan.task.reviewedTaskId': string;
    'plan.task.sourceTaskId': string;
    'plan.task.sourceFindingIds': string;
    'plan.task.coverageOf': string;
    'plan.task.listHint': string;
    'plan.task.unassigned': string;
    'plan.unsaved': string;
    'plan.save': string;
    'plan.saving': string;
    'plan.remove': string;
    'plan.removed': string;
    'plan.removeConfirm': string;
    'plan.removeWarning': string;
    'plan.cancel': string;
    'plan.addTask': string;
    'plan.adding': string;
    'plan.taskAdded': string;
    'plan.newTask': string;
    'plan.newTaskLabel': string;
    'plan.readySummary': string;
    'plan.flow.aria': string;
    'plan.flow.review': string;
    'plan.flow.spawn': string;
    'plan.flow.run': string;
    'plan.members.title': string;
    'plan.members.count': string;
    'plan.members.empty': string;
    'plan.tasks.title': string;
    'plan.tasks.count': string;
    'plan.tasks.empty': string;
    'plan.dependencies.none': string;
    'plan.dependencies.count': string;
    'plan.approve': string;
    'plan.approving': string;
    'plan.approveTitle': string;
    'plan.approveHint': string;
    'plan.approveConfirmTitle': string;
    'plan.approveWarning': string;
    'plan.approveConfirm': string;
    'plan.continue': string;
    'plan.returnToChat': string;
    'plan.feedbackTitle': string;
    'plan.feedbackHint': string;
    'plan.discard': string;
    'plan.discardConfirmTitle': string;
    'plan.discardWarning': string;
    'plan.discardConfirm': string;
    'plan.discarding': string;
    'plan.pendingEdits': string;
    'plan.saved': string;
    'plan.failed': string;
    'team.stats.members': string;
    'team.stats.completed': string;
    'team.stats.messages': string;
    'delegation.aria': string;
    'captain.name': string;
    'captain.role': string;
    'captain.summary': string;
    'captain.summary.staged': string;
    'captain.summary.awaitingFeedback': string;
    'captain.summary.discarded': string;
    'captain.summary.withTakeover': string;
    'captain.state.working': string;
    'captain.state.takeover': string;
    'captain.state.collected': string;
    'captain.state.waiting': string;
    'captain.state.staged': string;
    'captain.state.awaitingFeedback': string;
    'captain.state.discarded': string;
    'captain.state.settled': string;
    'members.toggle': string;
    'members.collapse': string;
    'members.expand': string;
    'members.empty': string;
    'assignment.label': string;
    'assignment.staged': string;
    'assignment.discarded': string;
    'assignment.empty': string;
    'archive.label': string;
    'archive.discardedLabel': string;
    'settings.title': string;
    'settings.intro': string;
    'settings.state.loading': string;
    'settings.state.unavailable': string;
    'settings.state.readOnly': string;
    'settings.write.saving': string;
    'settings.write.error': string;
    'settings.write.retry': string;
    'settings.write.noRevision': string;
    'settings.delegation.title': string;
    'settings.delegation.help': string;
    'settings.delegation.teams.label': string;
    'settings.delegation.teams.description': string;
    'settings.delegation.native.label': string;
    'settings.delegation.native.description': string;
    'settings.catalog.loading': string;
    'settings.catalog.empty': string;
    'settings.catalog.error': string;
    'settings.catalog.retry': string;
    'settings.profiles.title': string;
    'settings.profiles.help': string;
    'settings.profiles.loading': string;
    'settings.profiles.unsupportedPersistedVersion': string;
    'settings.profiles.bridgeUnavailable': string;
    'settings.profiles.error': string;
    'settings.profiles.saved': string;
    'settings.profiles.restart': string;
    'settings.profiles.listAria': string;
    'settings.profiles.new': string;
    'settings.profiles.copy': string;
    'settings.profiles.delete': string;
    'settings.profiles.builtIn': string;
    'settings.profiles.custom': string;
    'settings.profiles.name': string;
    'settings.profiles.rename': string;
    'settings.profiles.invalidName': string;
    'settings.profiles.duplicateName': string;
    'settings.profiles.restore': string;
    'settings.profiles.unsaved': string;
    'settings.profiles.save': string;
    'settings.profiles.saving': string;
    'settings.profiles.empty': string;
    'settings.profiles.description': string;
    'settings.profiles.protocol': string;
    'settings.profiles.executionPrompt': string;
    'settings.profiles.taskPlanning': string;
    'settings.profiles.captain': string;
    'settings.profiles.captainHelp': string;
    'settings.profiles.seed': string;
    'settings.profiles.seedHelp': string;
    'settings.profiles.captainTasksHint': string;
    'settings.profiles.members': string;
    'settings.profiles.member': string;
    'settings.profiles.memberName': string;
    'settings.profiles.memberRole': string;
    'settings.profiles.memberProvider': string;
    'settings.profiles.memberModel': string;
    'settings.profiles.followCaptain': string;
    'settings.profiles.chooseModel': string;
    'settings.profiles.defaultValue': string;
    'settings.profiles.reasoning.title': string;
    'settings.profiles.reasoning.target-default.label': string;
    'settings.profiles.reasoning.route-aware.label': string;
    'settings.profiles.reasoning.explicit.label': string;
    'settings.profiles.reasoning.effort': string;
    'settings.profiles.reasoning.noEfforts': string;
    'settings.profiles.explicitCatalogRequired': string;
    'settings.profiles.fallbackCatalogRequired': string;
    'settings.profiles.memberPrompt': string;
    'settings.profiles.memberFallback': string;
    'settings.profiles.profileFallback': string;
    'settings.profiles.fallbackProvider': string;
    'settings.profiles.fallbackModel': string;
    'settings.profiles.noFallback': string;
    'settings.profiles.unavailable': string;
    'settings.profiles.addMember': string;
    'settings.profiles.remove': string;
    'settings.profiles.tasks': string;
    'settings.profiles.newTask': string;
    'settings.profiles.taskId': string;
    'settings.profiles.taskSubject': string;
    'settings.profiles.taskDescription': string;
    'settings.profiles.taskAssignee': string;
    'settings.profiles.chooseAssignee': string;
    'settings.profiles.taskDependencies': string;
    'settings.profiles.commaSeparated': string;
    'settings.profiles.addTask': string;
    'settings.profiles.reviewPolicy': string;
    'settings.profiles.requirementsMinRounds': string;
    'settings.profiles.requirementsMaxRounds': string;
    'settings.profiles.codeMaxRounds': string;
    'settings.profiles.maxRepairAttempts': string;
    'settings.profiles.requiredReviewers': string;
};
/** AgentTeams namespace key union. */
export type AgentTeamsLocaleKey = keyof typeof zh;
/** English dictionary, checked complete against the Chinese source key set. */
export declare const en: {
    'card.memberCount': string;
    'action.openActivityPanel': string;
    'activity.panelButton': string;
    'activity.badgeAria': string;
    'activity.panelAria': string;
    'activity.title': string;
    'activity.float': string;
    'activity.dockRight': string;
    'activity.collapse': string;
    'activity.empty': string;
    'team.stop': string;
    'team.stopped': string;
    'team.stopTitle': string;
    'team.stopDescription': string;
    'team.stopCancel': string;
    'team.stopConfirm': string;
    'team.stopping': string;
    'team.stopFailed': string;
    'team.stopRequestFailed': string;
    'team.discarded': string;
    'format.listSeparator': string;
    'task.status.pending': string;
    'task.status.claimed': string;
    'task.status.inProgress': string;
    'task.status.completed': string;
    'task.status.failed': string;
    'task.status.cancelled': string;
    'task.status.notRun': string;
    'member.state.working': string;
    'member.state.failed': string;
    'member.state.waiting': string;
    'member.state.delivered': string;
    'member.state.left': string;
    'member.state.removed': string;
    'member.state.pending': string;
    'member.state.unassigned': string;
    'member.state.staged': string;
    'member.state.notCreated': string;
    'member.state.stopped': string;
    'member.status.executing': string;
    'member.status.executingModel': string;
    'member.status.working': string;
    'member.status.waitingOn': string;
    'member.status.waitingPrerequisite': string;
    'member.status.waitingAssignment': string;
    'member.status.delivered': string;
    'member.status.idle': string;
    'member.status.unknown': string;
    'member.status.staged': string;
    'member.status.settled': string;
    'member.status.discarded': string;
    'member.status.stopped': string;
    'task.assignee.unclaimed': string;
    'task.summary.waitingBreakdown': string;
    'task.summary.staged': string;
    'task.summary.discarded': string;
    'task.summary.allDelivered': string;
    'task.summary.ended': string;
    'task.summary.blockedAndRunning': string;
    'task.summary.more': string;
    'task.summary.running': string;
    'task.summary.ready': string;
    'task.summary.blocked': string;
    'task.summary.failedSettled': string;
    'task.summary.waitingSchedule': string;
    'progress.aria': string;
    'progress.title': string;
    'progress.running': string;
    'progress.blocked': string;
    'progress.delivered': string;
    'dependency.aria': string;
    'dependency.parallel': string;
    'dependency.title': string;
    'dependency.hint.parallel': string;
    'dependency.hint.chain': string;
    'dependency.hint.pinned': string;
    'task.runningAria': string;
    'task.model': string;
    'member.model': string;
    'task.detail.completed': string;
    'task.detail.noPrerequisite': string;
    'task.detail.ready': string;
    'task.detail.waitingOn': string;
    'task.detail.notRun': string;
    'task.detail.noDownstream': string;
    'task.detail.unlocks': string;
    'team.ended': string;
    'plan.badge': string;
    'plan.title': string;
    'plan.description': string;
    'plan.member.role': string;
    'plan.member.provider': string;
    'plan.member.model': string;
    'plan.member.reasoning': string;
    'plan.member.reasoningHint': string;
    'plan.model.choose': string;
    'plan.model.currentUnavailable': string;
    'plan.model.route': string;
    'plan.model.defaultReasoning': string;
    'plan.model.providerDefault': string;
    'plan.model.modelDefault': string;
    'plan.model.triggerAria': string;
    'plan.model.back': string;
    'plan.model.loading': string;
    'plan.model.empty': string;
    'plan.model.partialFailure': string;
    'plan.model.retry': string;
    'plan.member.prompt': string;
    'plan.member.roleFallback': string;
    'plan.task.subject': string;
    'plan.task.description': string;
    'plan.task.assignee': string;
    'plan.task.dependencies': string;
    'plan.task.dependenciesHint': string;
    'plan.task.kind': string;
    'plan.task.kind.work': string;
    'plan.task.kind.requirements': string;
    'plan.task.kind.implementation': string;
    'plan.task.kind.verification': string;
    'plan.task.kind.review': string;
    'plan.task.kind.repair': string;
    'plan.task.kind.integration': string;
    'plan.task.round': string;
    'plan.task.objective': string;
    'plan.task.inScope': string;
    'plan.task.outOfScope': string;
    'plan.task.acceptance': string;
    'plan.task.verify': string;
    'plan.task.deliverables': string;
    'plan.task.nonGoals': string;
    'plan.task.reviewedTaskId': string;
    'plan.task.sourceTaskId': string;
    'plan.task.sourceFindingIds': string;
    'plan.task.coverageOf': string;
    'plan.task.listHint': string;
    'plan.task.unassigned': string;
    'plan.unsaved': string;
    'plan.save': string;
    'plan.saving': string;
    'plan.remove': string;
    'plan.removed': string;
    'plan.removeConfirm': string;
    'plan.removeWarning': string;
    'plan.cancel': string;
    'plan.addTask': string;
    'plan.adding': string;
    'plan.taskAdded': string;
    'plan.newTask': string;
    'plan.newTaskLabel': string;
    'plan.readySummary': string;
    'plan.flow.aria': string;
    'plan.flow.review': string;
    'plan.flow.spawn': string;
    'plan.flow.run': string;
    'plan.members.title': string;
    'plan.members.count': string;
    'plan.members.empty': string;
    'plan.tasks.title': string;
    'plan.tasks.count': string;
    'plan.tasks.empty': string;
    'plan.dependencies.none': string;
    'plan.dependencies.count': string;
    'plan.approve': string;
    'plan.approving': string;
    'plan.approveTitle': string;
    'plan.approveHint': string;
    'plan.approveConfirmTitle': string;
    'plan.approveWarning': string;
    'plan.approveConfirm': string;
    'plan.continue': string;
    'plan.returnToChat': string;
    'plan.feedbackTitle': string;
    'plan.feedbackHint': string;
    'plan.discard': string;
    'plan.discardConfirmTitle': string;
    'plan.discardWarning': string;
    'plan.discardConfirm': string;
    'plan.discarding': string;
    'plan.pendingEdits': string;
    'plan.saved': string;
    'plan.failed': string;
    'team.stats.members': string;
    'team.stats.completed': string;
    'team.stats.messages': string;
    'delegation.aria': string;
    'captain.name': string;
    'captain.role': string;
    'captain.summary': string;
    'captain.summary.staged': string;
    'captain.summary.awaitingFeedback': string;
    'captain.summary.discarded': string;
    'captain.summary.withTakeover': string;
    'captain.state.working': string;
    'captain.state.takeover': string;
    'captain.state.collected': string;
    'captain.state.waiting': string;
    'captain.state.staged': string;
    'captain.state.awaitingFeedback': string;
    'captain.state.discarded': string;
    'captain.state.settled': string;
    'members.toggle': string;
    'members.collapse': string;
    'members.expand': string;
    'members.empty': string;
    'assignment.label': string;
    'assignment.staged': string;
    'assignment.discarded': string;
    'assignment.empty': string;
    'archive.label': string;
    'archive.discardedLabel': string;
    'settings.title': string;
    'settings.intro': string;
    'settings.state.loading': string;
    'settings.state.unavailable': string;
    'settings.state.readOnly': string;
    'settings.write.saving': string;
    'settings.write.error': string;
    'settings.write.retry': string;
    'settings.write.noRevision': string;
    'settings.delegation.title': string;
    'settings.delegation.help': string;
    'settings.delegation.teams.label': string;
    'settings.delegation.teams.description': string;
    'settings.delegation.native.label': string;
    'settings.delegation.native.description': string;
    'settings.catalog.loading': string;
    'settings.catalog.empty': string;
    'settings.catalog.error': string;
    'settings.catalog.retry': string;
    'settings.profiles.title': string;
    'settings.profiles.help': string;
    'settings.profiles.loading': string;
    'settings.profiles.unsupportedPersistedVersion': string;
    'settings.profiles.bridgeUnavailable': string;
    'settings.profiles.error': string;
    'settings.profiles.saved': string;
    'settings.profiles.restart': string;
    'settings.profiles.listAria': string;
    'settings.profiles.new': string;
    'settings.profiles.copy': string;
    'settings.profiles.delete': string;
    'settings.profiles.builtIn': string;
    'settings.profiles.custom': string;
    'settings.profiles.name': string;
    'settings.profiles.rename': string;
    'settings.profiles.invalidName': string;
    'settings.profiles.duplicateName': string;
    'settings.profiles.restore': string;
    'settings.profiles.unsaved': string;
    'settings.profiles.save': string;
    'settings.profiles.saving': string;
    'settings.profiles.empty': string;
    'settings.profiles.description': string;
    'settings.profiles.protocol': string;
    'settings.profiles.executionPrompt': string;
    'settings.profiles.taskPlanning': string;
    'settings.profiles.captain': string;
    'settings.profiles.captainHelp': string;
    'settings.profiles.seed': string;
    'settings.profiles.seedHelp': string;
    'settings.profiles.captainTasksHint': string;
    'settings.profiles.members': string;
    'settings.profiles.member': string;
    'settings.profiles.memberName': string;
    'settings.profiles.memberRole': string;
    'settings.profiles.memberProvider': string;
    'settings.profiles.memberModel': string;
    'settings.profiles.followCaptain': string;
    'settings.profiles.chooseModel': string;
    'settings.profiles.defaultValue': string;
    'settings.profiles.reasoning.title': string;
    'settings.profiles.reasoning.target-default.label': string;
    'settings.profiles.reasoning.route-aware.label': string;
    'settings.profiles.reasoning.explicit.label': string;
    'settings.profiles.reasoning.effort': string;
    'settings.profiles.reasoning.noEfforts': string;
    'settings.profiles.explicitCatalogRequired': string;
    'settings.profiles.fallbackCatalogRequired': string;
    'settings.profiles.memberPrompt': string;
    'settings.profiles.memberFallback': string;
    'settings.profiles.profileFallback': string;
    'settings.profiles.fallbackProvider': string;
    'settings.profiles.fallbackModel': string;
    'settings.profiles.noFallback': string;
    'settings.profiles.unavailable': string;
    'settings.profiles.addMember': string;
    'settings.profiles.remove': string;
    'settings.profiles.tasks': string;
    'settings.profiles.newTask': string;
    'settings.profiles.taskId': string;
    'settings.profiles.taskSubject': string;
    'settings.profiles.taskDescription': string;
    'settings.profiles.taskAssignee': string;
    'settings.profiles.chooseAssignee': string;
    'settings.profiles.taskDependencies': string;
    'settings.profiles.commaSeparated': string;
    'settings.profiles.addTask': string;
    'settings.profiles.reviewPolicy': string;
    'settings.profiles.requirementsMinRounds': string;
    'settings.profiles.requirementsMaxRounds': string;
    'settings.profiles.codeMaxRounds': string;
    'settings.profiles.maxRepairAttempts': string;
    'settings.profiles.requiredReviewers': string;
};
/** Translation function consumed by pure view helpers. */
export type AgentTeamsTranslate = (key: AgentTeamsLocaleKey, params?: Record<string, unknown>) => string;
