...
interface StatusRenderTeam {
  ...
  status_summary_unchanged?: boolean
}

export function statusFingerprint(value: JsonValue): string {
  const team = value as unknown as StatusRenderTeam
  const summary = {
    ...
    member_inboxes: Object.fromEntries(
      Object.entries(team.member_inboxes).map(([name, inbox]) => [name, {
        count: inbox.count,
...
export function renderStatus(value: JsonValue, detail: StatusRenderDetail = 'summary'): string {
  const team = value as unknown as StatusRenderTeam
  return detail === 'full' ? renderFullStatus(team) : renderSummaryStatus(team)
}

function statusFlags(team: StatusRenderTeam): string[] {
  return [
...
function renderSummaryStatus(team: StatusRenderTeam): string {
...
function renderFullStatus(team: StatusRenderTeam): string {
  const flags = statusFlags(team)
  const lines: string[] = [
