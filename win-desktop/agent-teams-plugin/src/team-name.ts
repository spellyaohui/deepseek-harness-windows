const SENSITIVE_DESCRIPTION = /(?:患者|病人|病患|姓名|病例|病案|病历(?:号)?|住院(?:号)?|门诊(?:号)?|就诊(?:号)?|检查号|化验单|检验单|影像号|报告号|诊断|处方|医嘱|身份证(?:号)?|医保(?:号|卡)?|床号|手机号|联系电话|出生日期|patient|medical\s*(?:record|history)|health\s*record|mrn|inpatient|outpatient|diagnosis|dicom(?:\s*uid)?|study\s*uid|series\s*uid|sop\s*uid|accession|token|api.?key|secret|password|credential|bearer|private.?key|client.?secret)/iu
