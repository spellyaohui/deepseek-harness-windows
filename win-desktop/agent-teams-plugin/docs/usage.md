...
|---|---|
| `ctx.tools` 注册表 | 注册原有 13 个 `agent_teams_*` 工具；按会话身份限制模型可见工具 |
...
| `ctx.systemPrompt.section()` | 为队长和成员分别提供稳定的固定核心策略提示段 |
...
| `agent_teams_create_task` | 创建任务，支持 `dependencies` 依赖声明与 `assignee` 指派；成员名指向成员，`captain` 指向队长，空值进入共享任务池 |
