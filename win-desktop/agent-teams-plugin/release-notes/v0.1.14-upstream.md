...
> Windows fork compatibility: ordinary AgentTeams requests retain the existing
> immediate-execution default. The staged review flow is available through
> explicit `approval=required` requests and captain-planning profiles; local
> settings, Team/Native routing, and member-claim compatibility remain active.
...
> Windows fork 兼容说明：普通 AgentTeams 请求继续保持原有的即时执行默认行为；显式
> `approval=required` 或队长规划 profile 才进入执行前审查流程。本项目的设置、Team/Native
> 路由和成员认领兼容能力继续保留。
